// Supabase config agora vem do config.js
...
window.debugAuth = {
    getToken: () => supabase.auth.getSession().then(x => {
        const token = x.data?.session?.access_token;
        console.log("🔑 Token:", token);
        return token;
    }),
    getUser: () => console.log("👤 Current User:", getCurrentUser()),
    checkSession: () => supabase.auth.getSession().then(x => console.log("📋 Session:", x.data?.session))
};